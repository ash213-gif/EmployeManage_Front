...
export default function Message({ selectedAdmin }) {
  ...
  const handleSubmitMessage = async () => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        fetchMessages(); // Optionally fetch messages again to update the view
      } else {
  ...
  const fetchMessages = async () => {
    if (receiverId) {
      try {
        const response = await axios.post(`${GlobarRenderUrl}/getmessages`, {
          senderId: userId,
  ...
  useEffect(() => {
    fetchMessages(); // Fetch messages when the component mounts or receiverId changes
  }, [receiverId]);
